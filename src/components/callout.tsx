...
export function Callout({
  ...
  return (
    <div className={cn("relative my-10 w-full rounded-xl border p-4 shadow", className)}>
...
export function CalloutTitle({ children }: { children: ReactNode }) {
  return <h5 className="mt-0! font-medium! leading-none tracking-tight">{children}</h5>;
}
export function CalloutDescription({ children }: { children: ReactNode }) {
  return <div className="prose-p:text-rose-500 text-sm [&_p]:leading-relaxed">{children}</div>;
}
